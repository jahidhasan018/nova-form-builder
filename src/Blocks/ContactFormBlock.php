...
class ContactFormBlock extends AbstractBlock {
	public function __construct( string $plugin_url, string $plugin_path, private SettingsRepositoryInterface $settings ) {
	...
	public function render( array $attributes = array() ): string {
		...
		ob_start();
		?>
		<form id="<?php echo esc_attr( $form_id ); ?>" class="nova-form-builder__contact-form nova-style-<?php echo esc_attr( $preset ); ?>" method="post" data-endpoint="<?php echo esc_url( $rest_url ); ?>">
			...
			<p style="display:none !important;"><input name="website" type="text" tabindex="-1" autocomplete="off" /></p>
			<p><label for="<?php echo esc_attr( $form_id . '-name' ); ?>"><?php esc_html_e( 'Name', 'nova-form-builder' ); ?></label><input id="<?php echo esc_attr( $form_id . '-name' ); ?>" name="name" type="text" required /></p>
			<p><label for="<?php echo esc_attr( $form_id . '-email' ); ?>"><?php esc_html_e( 'Email', 'nova-form-builder' ); ?></label><input id="<?php echo esc_attr( $form_id . '-email' ); ?>" name="email" type="email" required /></p>
			<p><label for="<?php echo esc_attr( $form_id . '-message' ); ?>"><?php esc_html_e( 'Message', 'nova-form-builder' ); ?></label><textarea id="<?php echo esc_attr( $form_id . '-message' ); ?>" name="message" rows="5" required></textarea></p>
